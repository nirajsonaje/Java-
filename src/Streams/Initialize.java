...
public class Initialize {
	...
	private static void builder() {
		Stream.<String>builder().add("cat").add(" dog").add(" bear").build().forEach(System.out::print); // prints: cat
																											// dog bear
	...
	private static void generate() {
		...
		Stream.generate(() -> new Random().nextDouble()).limit(5).forEach(System.out::println); // prints:
																								// 0.38575117472619247
																								// 0.5055765386778835
																								// 0.6528038976983277
																								// 0.4422354489467244
	...
	private static void emptyOfAndOfNullable() {
		...
		Stream.of(stream1, stream2, stream3, stream4).forEach(System.out::print);
	...
	private static void printList3(List<String> list) {
		Stream.ofNullable(list).flatMap(e -> e.stream()).forEach(System.out::print);
	}

	private static void printList4(List<String> list) {
		Stream.ofNullable(list).flatMap(Collection::stream).forEach(System.out::print);
	}
